...
export default function StoreHomePage() {
  ...
  return (
    <div className="min-h-screen bg-white relative">
            ...
            <div className="space-y-8 animate-on-scroll opacity-0 transform translate-x-[-50px]">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-6 lg:gap-8 pt-6 border-t border-gray-200">
                  {[{number:'12K+',label:'Vận Động Viên'},{number:'50+',label:'Giải Đấu'},{number:'99%',label:'Hài Lòng'}].map((stat,index)=>(
      ...
      <section className="py-20 lg:py-24 px-6 bg-white relative overflow-hidden">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[{icon:'⚡',title:'HIỆU SUẤT CAO',description:'Công nghệ tiên tiến nhất',color:'from-gray-700 to-gray-900'},
              {icon:'🛡️',title:'BỀN VỮNG',description:'Chất liệu cao cấp',color:'from-gray-600 to-gray-800'},
              {icon:'🎯',title:'THIẾT KẾ TỐI ƯU',description:'Cho vận động tối đa',color:'from-gray-800 to-gray-900'}].map((feature,index)=>(
              <div key={index} className="group text-center animate-on-scroll opacity-0 transform translate-y-8" style={{ animationDelay: `${index*200}ms` }}>
                <div className={`relative bg-white rounded-2xl p-8 border border-gray-200 transition-all duration-500 group-hover:shadow-xl group-hover:scale-105 h-full`}>
                  <div className={`w-20 h-20 bg-gradient-to-br ${feature.color} rounded-2xl flex items-center justify-center mx-auto mb-6 transform group-hover:scale-110 transition duration-500`}>
      ...
      {/* ================== Newsletter Section ================== */}
      <section className="py-20 lg:py-24 px-6 bg-gray-50 relative overflow-hidden">
        <div className="max-w-3xl mx-auto text-center animate-on-scroll opacity-0 transform translate-y-8">
          <h2 className="text-3xl lg:text-4xl font-black text-gray-900 mb-6 break-words">
            Đăng ký nhận tin khuyến mãi
          </h2>
          <p className="text-gray-600 mb-8">
            Nhận thông tin sản phẩm mới và ưu đãi độc quyền trực tiếp vào email
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <input 
              type="email" 
              placeholder="Nhập email của bạn"
              className="px-6 py-4 rounded-lg border border-gray-300 focus:ring-2 focus:ring-gray-900 focus:outline-none w-full sm:w-auto"
            />
            <button className="bg-gray-900 text-white px-8 py-4 rounded-lg font-bold hover:bg-gray-800 transition-all duration-300 w-full sm:w-auto">
              Đăng ký
            </button>
          </div>
        </div>
